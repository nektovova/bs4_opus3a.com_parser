...
def create_table():
    ...
    table_query = '''CREATE TABLE IF NOT EXISTS items
                (name text, singer_name text, category text, item_format text, release_year text, barcode text, price int, item_url text, updated_ts text, show int)'''
...
def add_data(name, singer_name, category, item_format, release_year, barcode, price, item_url):
    connection = sqlite3.connect(db_name) 
    # student list 
    cursor= connection.cursor()
    ...
    if result:  # result could be None or tuple (record)
        ...
        q = f'INSERT INTO items VALUES ("{name}","{singer_name}","{category}","{item_format}","{release_year}","{barcode}","{price}","{item_url}","{updated_ts}", 0)'
